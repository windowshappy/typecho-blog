...
class LoginDesigner_Plugin implements Typecho_Plugin_Interface
{
    ...
    {
        \Typecho\Plugin::factory('admin/header.php')->header = array('LoginDesigner_Plugin', 'render');
        \Typecho\Plugin::factory('admin/footer.php')->end = array('LoginDesigner_Plugin', 'footerjs');
    }
...
$zz10='<div class="zz">空白样式<br>不使用内置样式</div>'; 
 ...
 $bgfengge = new \Typecho\Widget\Helper\Form\Element\Radio(
'bgfengge', array(
  ...
  'kongbai' => _t('<div class="kuai"><img src="'.$url.'/images/kongbai.jpg" loading="lazy">'.$zz10.'</div>'),
    ...
    $bgUrl = new \Typecho\Widget\Helper\Form\Element\Text('bgUrl', NULL, NULL, _t('自定义背景图'), _t('选中上方的基础样式后，可以在这里填写图片地址自定义背景图；<b>注意</b>：带有【动态】标识的风格不支持自定义背景图。'));
    $form->addInput($bgUrl);
    
    $diycss = new \Typecho\Widget\Helper\Form\Element\Textarea('diycss', NULL,NULL,'自定义样式', _t('上边的样式选择【空白样式】，然后在这里自己写css美化注册/登录页面；<b>注意</b>：该功能与【自定义背景图】功能冲突，使用该功能时如果想设置背景图请写css里面。
...
$url=Helper::options()->pluginUrl.'/LoginDesigner/';
$skin=\Typecho\Widget::widget('Widget_Options')->plugin('LoginDesigner')->bgfengge;
$diycss=\Typecho\Widget::widget('Widget_Options')->plugin('LoginDesigner')->diycss;
if($skin=='kongbai'){
...
$bgUrl=\Typecho\Widget::widget('Widget_Options')->plugin('LoginDesigner')->bgUrl;
$zidingyi="";
if($bgUrl){
