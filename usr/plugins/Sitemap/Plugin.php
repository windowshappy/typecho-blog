...
class Sitemap_Plugin implements Typecho_Plugin_Interface
{
     ...
     * @access public
     * @return void
     * @throws \Typecho\Plugin_Exception
     */
    public static function activate()
    {
        ...
        \Typecho\Plugin::factory('Widget_Contents_Post_Edit')->finishPublish = array('Sitemap_Plugin', 'auto');
        \Typecho\Plugin::factory('Widget_Contents_Page_Edit')->finishPublish = array('Sitemap_Plugin', 'auto');
    }
     ...
     * 
     * @access public
     * @param \Typecho\Widget_Helper_Form $form 配置面板
     * @return void
     */
    public static function config(\Typecho\Widget\Helper\Form $form)
    {
        ...
        $sitemap_cachetime = new Typecho\Widget\Helper\Form\Element\Text('sitemap_cachetime', NULL, '7', _t('Sitemap 缓存时间'), '单位（天）');
        $form->addInput($sitemap_cachetime);
        $baidu_url = new \Typecho\Widget\Helper\Form\Element\Text('baidu_url', NULL, NULL, _t('百度推送接口 URL'), '请在<a href="https://ziyuan.baidu.com/">百度搜索资源平台</a>查看，目前仅支持普通收录');
        $form->addInput($baidu_url);
        $web_token = new \Typecho\Widget\Helper\Form\Element\Text('api_token', NULL, \Typecho\Common::randString(32), _t('API token'), '自动生成无需修改，留空则不开启API功能<br>' . Helper::options()->index . '/sitemap-api?sitemap=[update]&push=[main/all/new]&token=[API_TOKEN]');
        $form->addInput($web_token);
        $AutoPush = new \Typecho\Widget\Helper\Form\Element\Radio('AutoPush', array(0 => _t('不开启'), 1 => _t('开启')), 0, _t('自动推送文章'), '发布文章自动推送当前文章，自定义文章路径需使用wordpress风格，或其他带有{slug}的路径');
        $form->addInput($AutoPush);
        $AutoSitemap = new \Typecho\Widget\Helper\Form\Element\Radio('AutoSitemap', array(0 => _t('不开启'), 1 => _t('开启')), 0, _t('自动更新Sitemap'), '发布文章更新Sitemap，可能降低发布速度不推荐开启');
        $form->addInput($AutoSitemap);
        $PluginLog = new \Typecho\Widget\Helper\Form\Element\Radio('PluginLog', array(0 => _t('不开启'), 1 => _t('开启')), 0, _t('插件日志'), '将推送日志和Sitemap更新日志存入插件目录下');
        $form->addInput($PluginLog);
        $Btn = new \Typecho\Widget\Helper\Form\Element\Submit();
        ...
        $Btn->input->setAttribute('formaction', \Typecho\Common::url('/options-plugin.php?config=Sitemap&action=update_sitemap', Helper::options()->adminUrl));
        $form->addItem($Btn);
    }
    /**
     * 更新 sitemap
    ...
    public static function update_sitemap()
    {
        ...
        header("location:" . \Typecho\Common::url('/options-plugin.php?config=Sitemap', Helper::options()->adminUrl));
    }
    /**
     * 插件实现方法
     * 
     ...
     * 
     * @access public
     * @param \Typecho\Widget_Helper_Form $form
     * @return void
     */
    ...
    public static function auto($contents, $class)
    {
        ...
        $url = \Typecho\Common::url($path_info, Helper::options()->index);
        $urls = array(
            $url,
        );
        //检查自动推送文章开关
